...
func (q *Queries) CreateTrip(
	ctx context.Context,
	pool pgxpool.Pool,
	params spec.PostTripsJSONBody,
) (uuid.UUID, error) {
	tx, err := pool.Begin(ctx)
	if err != nil {
		return uuid.UUID{}, fmt.Errorf("pgstore: falied to begin trx for CreateTrip: %w", err)
	}
	...
	if err != nil {
		return uuid.UUID{}, fmt.Errorf("pgstore: falied to insert trip for CreateTrip: %w", err)
	}
	...
	if err := tx.Commit(ctx); err != nil {
		return uuid.UUID{}, fmt.Errorf("pgstore: falied to commit trx for CreateTrip: %w", err)
	}
