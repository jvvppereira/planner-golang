...
const creatTripLink = `-- name: CreatTripLink :one
INSERT INTO links
...
type CreatTripLinkParams struct {
	TripID uuid.UUID
	Title  string
	Url    string
}

func (q *Queries) CreatTripLink(ctx context.Context, arg CreatTripLinkParams) (uuid.UUID, error) {
	row := q.db.QueryRow(ctx, creatTripLink, arg.TripID, arg.Title, arg.Url)
	var id uuid.UUID
	err := row.Scan(&id)
	return id, err
}
...
const getTripActivites = `-- name: GetTripActivites :many
SELECT
...
func (q *Queries) GetTripActivites(ctx context.Context, tripID uuid.UUID) ([]Activity, error) {
	rows, err := q.db.Query(ctx, getTripActivites, tripID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const invitePaticipantToTrip = `-- name: InvitePaticipantToTrip :one
...
type InvitePaticipantToTripParams struct {
	TripID uuid.UUID
	Email  string
}

func (q *Queries) InvitePaticipantToTrip(ctx context.Context, arg InvitePaticipantToTripParams) (uuid.UUID, error) {
	row := q.db.QueryRow(ctx, invitePaticipantToTrip, arg.TripID, arg.Email)
	var id uuid.UUID
	err := row.Scan(&id)
	return id, err
}
